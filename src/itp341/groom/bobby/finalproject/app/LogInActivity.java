...
public class LogInActivity extends Activity {
	...
	public void loadDataPlayer(long id, Player play) {
		...
		if (c != null) {
			if (c.moveToFirst()) {	// otherwise something weird happened but necesssary for android
				...
				play = new Player(c.getLong(TABLE_PLAYERS.COLUMN_ID),
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == Activity.RESULT_OK) { // this means user pressed back
	...
	public void chooseDifficulty() {
		...
		i.putExtra(GameSetupActivity.EXTRA_PLAYER_ONE_INFO, p1);
		i.putExtra(GameSetupActivity.EXTRA_PLAYER_TWO_INFO, p2);
